...
class Repository:
    ...
    def get_groupby_df(self, df, grouping_dict, aggregation_dict):
    ...
    def get_raw_display2(self, display_id, pd_group=False):
        ...
        if type(row["aggregations"]) == float:
            if math.isnan(row["aggregations"]):
                df_gb, agg_df = self.get_groupby_df(raw_df,json.loads(row["grouping"]), None)
            else:
                df_gb, agg_df = self.get_groupby_df(raw_df,json.loads(row["grouping"]), json.loads(row["aggregations"]))
        else:
            df_gb, agg_df = self.get_groupby_df(raw_df,json.loads(row["grouping"]), json.loads(row["aggregations"]))
        if pd_group:
            return raw_df, df_gb
    ...
    def display_distance(self,id1, id2, method="all"):
        ...
        #try:
        return display_distance(dd1,dd2, method=method)
        #except:
        #    return None
