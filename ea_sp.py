...
train_tree_seqs, train_lumps, train_seq_y = generate_tree_sequences_train(train_x, train_seqs, train_aids, lump_graphs, train_y)
...
for i, seq in tqdm(enumerate(train_tree_seqs)):
    if len(seq) in train_seqs_by_length:
        ...
        train_seq_y_by_length[len(seq)].append(train_seq_y[i])
    else:
        train_seqs_by_length[len(seq)] = [seq]
        train_lumps_by_length[len(seq)] = [train_lumps[i]]
        train_seq_y_by_length[len(seq)] = [train_seq_y[i]]

# total_examples = 0
# length_count = 0
# for length in train_seq_y_by_length:
#     total_examples += len(train_seq_y_by_length[length])
#     length_count += 1

# batch_size = 1 * (total_examples // length_count)
# print(batch_size)
...
for _ in tqdm(range(5)):
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        dst_probs = calc_dst_probs(model, test_loaders)
        if test_ra3 > peak_ra3:
            pickle.dump(
                            dst_probs, 
                            open(f'./dst_probs/gine_seq_{task}_best_ra3_{seed}_{test_id}_{main_size}.pickle', 'wb'), 
        ...
        if test_mrr > peak_mrr:
            pickle.dump(
                            dst_probs, 
                            open(f'./dst_probs/gine_seq_{task}_best_mrr_{seed}_{test_id}_{main_size}.pickle', 'wb'), 
                            protocol=pickle.HIGHEST_PROTOCOL
                        )
            peak_mrr = test_mrr
